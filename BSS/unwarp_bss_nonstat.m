...
for k=1:Kmat
   ...
   indMOREAU(k) = amari(BMoreau,pileA(:,:,vectau(k)));
   k2 = length(vectauns(vectauns<=vectau(k)));
   indPSOBI(k) = amari(pile_Bsobins(:,:,k2),pileA(:,:,vectau(k)));
end

fprintf('Critère | SOBI    | p-SOBI  | Moreau  | JEFAS-BSS \n')
...
fprintf('Amari   | %.2f  | %.2f  | %.2f  | %.2f\n', mean(indSOBI),mean(indPSOBI),mean(indMOREAU),mean(indJEFAS))

figure;subplot(2,1,2);
plot(t(vectau),indJEFAS,t(vectau),indSOBI,'k--',t(vectau),indMOREAU,'g-.',t(vectau),indPSOBI,'r:','linewidth',2); grid on; axis tight;
xlabel('Time (s)'); ylabel('Amari index (dB)'); grid on; legend('JEFAS-BSS','SOBI','Moreau','p-SOBI');
%% Convergence
